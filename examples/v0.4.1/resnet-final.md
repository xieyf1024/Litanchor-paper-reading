...
| 关键限制 | 网络并非越深越好：1202-layer 网络的测试结果差于 110-layer 网络，作者认为这可能是小数据集上的过拟合，并指出 19.4M 参数对该数据集可能过大。 〔E-RN-LIMIT, E-RN-LIMIT-B, E-RN-RES-CF-B｜PDF p.7, p.8｜[打开 p.7](zotero://open-pdf/library/items/M8REGQUE?page=7)｜[打开 p.8](zotero://open-pdf/library/items/M8REGQUE?page=8)〕 |
...
#### Metric 3：top-1 / top-5 error

- **用途与定义**：ImageNet 分类实验同时使用 top-1 error 与 top-5 error 作为评价指标。 〔E-RN-METRIC-IM｜PDF p.4｜[打开 p.4](zotero://open-pdf/library/items/M8REGQUE?page=4)〕
- top-1 与 top-5 均为错误率，结果解读时不得与准确率混写。
- **成立条件、单位或适用限制**：ImageNet 分类任务；报告 error rate。

#### Metric 4：COCO mAP@[.5, .95]

- **用途与定义**：COCO 检测实验采用 COCO 标准指标 mAP@[.5, .95] 衡量检测性能。 〔E-RN-DETECT｜PDF p.8｜[打开 p.8](zotero://open-pdf/library/items/M8REGQUE?page=8)〕
- 该指标对应论文报告的 6.0% 绝对增益与 28% 相对改进。
- **成立条件、单位或适用限制**：COCO 目标检测；IoU 阈值范围 [.5, .95]。
...
- **主要发现**：在 CIFAR-10 上，110-layer ResNet 能够收敛；Table 6 报告其参数量为 1.7M、测试误差为 6.43（表中单位为 %）。 〔E-RN-RES-CF, E-RN-RES-CF-B｜PDF p.7｜[打开 p.7](zotero://open-pdf/library/items/M8REGQUE?page=7)〕
- **论证与细节**：
  - 论文强调这里关注极深网络的行为而非专门追逐 state of the art。
  - 结果与 FitNet、Highway 等深而窄网络进行参数和误差比较。
- **关键数值、比较或不确定性**：model depth 110-layer（CIFAR-10）；test error 6.43%（CIFAR-10）；parameter count 1.7M（CIFAR-10）
- **证据状态**：作者报告
- **成立条件与适用范围**：原文未说明
...
| --- | --- | --- | --- | --- | --- |
| Figure 2 | 该图直接呈现输入 x、残差分支 F(x) 与恒等 shortcut 的相加关系，是理解论文核心方法不可替代的示意图。 | Figure 2. Residual learning: a building block. | 核心方法图 | [PDF p.2](zotero://open-pdf/library/items/M8REGQUE?page=2) | 已查看原 PDF 裁图 |
| Figure 6 | 该图同时呈现 plain network 的退化、ResNet 的可优化性以及 110/1202 层模型的泛化差异。 | Figure 6. Training on CIFAR-10. Dashed lines denote training error, and bold lines denote testing error. Left: plain networks. The error of plain-110 is higher than 60% and not displayed. Middle: ResNets. Right: ResNets with 110 and 1202 layers. | 核心结果图 | [PDF p.8](zotero://open-pdf/library/items/M8REGQUE?page=8) | 已查看原 PDF 裁图 |

### Figure 2

![ResNet Figure 2](assets/resnet/figure-2.png)
...
![ResNet Figure 6](assets/resnet/figure-6.png)

- **选择理由**：该图同时呈现 plain network 的退化、ResNet 的可优化性以及 110/1202 层模型的泛化差异。
- **完整图题**：Figure 6. Training on CIFAR-10. Dashed lines denote training error, and bold lines denote testing error. Left: plain networks. The error of plain-110 is higher than 60% and not displayed. Middle: ResNets. Right: ResNets with 110 and 1202 layers.
- **正文讨论位置**：PDF p.7–8，Section 4.2
- **读图注意事项**：原文未说明
- [在 Zotero 打开原页](zotero://open-pdf/library/items/M8REGQUE?page=8)
...
- 网络并非越深越好：1202-layer 网络的测试结果差于 110-layer 网络，作者认为这可能是小数据集上的过拟合，并指出 19.4M 参数对该数据集可能过大。 〔E-RN-LIMIT, E-RN-LIMIT-B, E-RN-RES-CF-B｜PDF p.7, p.8｜[打开 p.7](zotero://open-pdf/library/items/M8REGQUE?page=7)｜[打开 p.8](zotero://open-pdf/library/items/M8REGQUE?page=8)〕
  - 两种深度的训练误差相近，但测试表现不同。
...
- 可继续检验“先重写优化目标、再增加模型容量”的设计原则：当直接学习目标映射困难时，是否能通过基准项加残差项的参数化降低优化难度。 〔E-RN-MAP, E-RN-HYP, E-RN-CONCL｜PDF p.2｜[打开 p.2](zotero://open-pdf/library/items/M8REGQUE?page=2)〕
  - 这是从本文残差重参数化抽象出的研究启发，不是作者已经验证的跨领域结论。
  - 迁移到其他问题时仍需重新定义基准映射、残差项和评价指标。
...
2. **Figure 6**：该图同时呈现 plain network 的退化、ResNet 的可优化性以及 110/1202 层模型的泛化差异。

#### 5 个值得学习的英文表达

- “We present a residual learning framework to ease the training of networks that are substantially deeper than those used previously.”——适合在摘要中直接说明方法及其目标。 〔E-RN-SUM｜PDF p.1｜[打开 p.1](zotero://open-pdf/library/items/M8REGQUE?page=1)〕
- “Driven by the significance of depth, a question arises: Is learning better networks as easy as stacking more layers?”——用于从研究重要性自然转入核心问题。 〔E-RN-Q｜PDF p.1｜[打开 p.1](zotero://open-pdf/library/items/M8REGQUE?page=1)〕
- “We hypothesize that it is easier to optimize the residual mapping than to optimize the original, unreferenced mapping.”——明确把机制解释限定为假设。 〔E-RN-HYP｜PDF p.2｜[打开 p.2](zotero://open-pdf/library/items/M8REGQUE?page=2)〕
- “Most remarkably, on the challenging COCO dataset we obtain a 6.0% increase in COCO’s standard metric ...”——用于突出跨任务实验中最重要的定量结果。 〔E-RN-DETECT｜PDF p.8｜[打开 p.8](zotero://open-pdf/library/items/M8REGQUE?page=8)〕
- “This strong evidence shows that the residual learning principle is generic, and we expect that it is applicable in other vision and non-vision problems.”——在同一句中区分已有证据与未来预期。 〔E-RN-CONCL｜PDF p.2｜[打开 p.2](zotero://open-pdf/library/items/M8REGQUE?page=2)〕
...
- **Residual mapping（残差映射）**：将目标映射 H(x) 改写为 F(x)+x，其中 F(x)=H(x)-x。 〔E-RN-MAP｜PDF p.2｜[打开 p.2](zotero://open-pdf/library/items/M8REGQUE?page=2)〕
- **Identity shortcut（恒等捷径连接）**：把输入直接传到加法节点，不增加额外参数或计算复杂度。 〔E-RN-SHORT｜PDF p.2｜[打开 p.2](zotero://open-pdf/library/items/M8REGQUE?page=2)〕
- **Degradation problem（退化问题）**：网络继续加深后，训练准确率趋于饱和并快速下降；作者明确区分其与过拟合。 〔E-RN-GAP, E-RN-DEGR-OVERFIT｜PDF p.1｜[打开 p.1](zotero://open-pdf/library/items/M8REGQUE?page=1)〕
- **Top-1 / top-5 error**：ImageNet 分类实验并列报告的两类错误率指标。 〔E-RN-METRIC-IM｜PDF p.4｜[打开 p.4](zotero://open-pdf/library/items/M8REGQUE?page=4)〕
- **mAP@[.5, .95]**：论文在 COCO 检测实验中使用的标准平均精度指标。 〔E-RN-DETECT｜PDF p.8｜[打开 p.8](zotero://open-pdf/library/items/M8REGQUE?page=8)〕
...
| E-RN-CON2 | We show that: 1) Our extremely deep residual nets are easy to optimize, but the counterpart “plain” nets (that simply stack layers) exhibit higher training error when the depth increases; 2) Our deep residual nets can easily enjoy accuracy gains from greatly increased depth, producing results substantially better than previous networks. | PDF p.2 / Introduction | C-RN-CON2 |
...
| E-RN-IMPL | We use SGD with a mini-batch size of 256. The learning rate starts from 0.1 and is divided by 10 when the error plateaus, and the models are trained for up to 60 × 104 iterations. We use a weight decay of 0.0001 and a momentum of 0.9. We do not use dropout [13], following the practice in [16]. | PDF p.4 / Implementation | C-RN-EXP |
| E-RN-RES-152 | Our 152-layer ResNet has a single-model top-5 validation error of 4.49%. This single-model result outperforms all previous ensemble results (Table 5). | PDF p.7 / ImageNet Classification | C-RN-RES-152 |
| E-RN-RES-ENS | We combine six models of different depth to form an ensemble (only with two 152-layer ones at the time of submitting). This leads to 3.57% top-5 error on the test set (Table 5). | PDF p.7 / ImageNet Classification | C-RN-RES-ENS |
| E-RN-RES-CF | This 110-layer network converges well (Fig. 6, middle). | PDF p.7 / CIFAR-10 and Analysis | C-RN-RES-CF |
| E-RN-RES-CF-B | method error (%) Maxout [9] 9.38 NIN [25] 8.81 DSN [24] 8.22 # layers # params FitNet [34] 19 2.5M 8.39 Highway [41, 42] 19 2.3M 7.54 (7.72±0.16) Highway [41, 42] 32 1.25M 8.80 ResNet 20 0.27M 8.75 ResNet 32 0.46M 7.51 ResNet 44 0.66M 7.17 ResNet 56 0.85M 6.97 ResNet 110 1.7M 6.43 (6.61±0.16) ResNet 1202 19.4M 7.93 Table 6. Classification error on the CIFAR-10 test set. | PDF p.7 / CIFAR-10 and Analysis | C-RN-RES-CF, C-RN-LIMIT |
| E-RN-LIMIT | We argue that this is because of overfitting. | PDF p.8 / Exploring Over 1000 Layers | C-RN-LIMIT |
...
| E-RN-DETECT | Most remarkably, on the challenging COCO dataset we obtain a 6.0% increase in COCO’s standard metric (mAP@[.5, .95]), which is a 28% relative improvement. | PDF p.8 / Object Detection on PASCAL and MS COCO | C-RN-RES-DET, C-RN-CONCL |

### 8.3 值得继续追踪的参考文献

**本模式未生成（仅 internalize 模式要求）**

<!-- litanchor:validation template=paper-template-final@1.0; run=20260724T114926Z-51b5de45eb0b; evidence=31; claims=39; status=completed_with_warnings; pdf_sha256=51b5de45eb0b558b19c3affe49503cff50cb170a32de602983d6e2ec286942a7 -->
