...
12. **Compose note.** Treat `assets/Paper Template.md` as the human-readable `Paper Template v1.0` content contract and render through the internal slot asset `assets/Paper Template - Runtime.md`. `skim` contains Section 1 only, `deep` contains Sections 1–6, and `internalize` contains Sections 1–8. All modes use the fixed note-frontmatter Schema, render the complete English title plus a faithful Chinese translation in `title`, use a concise Chinese semantic filename, and place the mode only in tags. Never present runtime slots or AI-only instructions as the user template.
