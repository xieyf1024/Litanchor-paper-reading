...
For a selected figure, run `scripts/pdf_figures.py` against the original PDF. Verify the physical page and caption, inspect the PNG, retain its JSON provenance manifest, embed it with a Vault-relative wikilink, and add a verified Zotero page link. The crop must pass edge, caption-boundary, source-hash, page, output-hash and human-review gates. When an explicit bounding box is needed, derive its lower edge from the caption text-block boundary and leave only a small blank margin; any following heading or body-text block inside the crop is contamination and blocks output. A structure parser may locate a candidate, but the published image must be cropped from the original PDF. Ambiguous, clipped or contaminated geometry blocks automatic embedding.

## 8. Build note identity and ledgers

Before materializing the ledgers, create one paper-independent `note_identity` object:

- `title_zh`: a faithful Chinese translation of the complete verified English title; keep it on one line and do not add interpretation.
- `filename_stem_zh`: a concise Chinese noun phrase that identifies the paper's subject or central contribution. Prefer 4–24 Chinese characters, omit authors, year, journal, DOI, generic suffixes such as “论文精读/阅读笔记”, and details that do not distinguish the paper. It may summarize the original title, but must not copy any example or paper-specific correction from Skill content.

The filename stem must contain Chinese, must not end in a period or space, and must exclude Obsidian/Windows-invalid characters. A deterministic validator blocks unsafe or non-Chinese stems. Keep the complete bilingual title in frontmatter; the filename is deliberately shorter.
...
The note frontmatter contains exactly `title`, `first_author`, `year`, `journal`, `doi`, `paper_type`, `keywords`, `source_coverage`, `locator_mode`, `validation_status`, `review_status`, `skill_version`, `template_version`, `created`, `updated`, and `tags`. Render `title` as one YAML literal string whose first line is the complete verified English title and whose second line is its faithful Chinese translation. Keep the full author list and Zotero/source identifiers in SourceBundle and sidecars, not in the displayed note. Keywords come only from the paper or Zotero and are joined by semicolons. Dates use `YYYY-MM-DD`: `created` is fixed when LitAnchor first generates the note successfully and never comes from Zotero-added, PDF-import, acquisition, or run-start metadata; `updated` records the current successful render or update. Never overwrite an existing note; any future update path must preserve both `created` and all user-added tags.
